...
pipeline {
  ...
  environment {
     APP_NAME = 'BuyRequest/report-service'
     WORKSPACE = '/home/Jenkins/workspace/BuyRequest/report-service'
  }
   agent none
   stages {
            ...
            steps {
                script {
                    sshagent (credentials: ['buyrequestdev']) {
                        ...
                        && cat .env \
                        && cd ../services/report-service && git checkout -f && git pull \
                        && cd /home/ubuntu/buy-request/docker && make report-service-restart && make nginx-restart \
                        && make report-service-logs-without-tail \
