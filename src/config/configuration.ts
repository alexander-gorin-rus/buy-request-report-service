...
const getClients = (): any => [
  ...
  },
  {
    ...
    options: {
      ...
      protoPath: [
        join(process.cwd(), 'protos/report-service/report.proto'),
        join(process.cwd(), 'protos/report-service/error.proto'),
      ],
...
export default () => ({
  applicationName: `${process.env.APPLICATION_NAME} REPORT SERVICE`,
  url: `${process.env.REPORT_SERVICE_HOST}:${process.env.REPORT_SERVICE_PORT}`,
